...
{
    public class VoeExtractor : Extractor
    {
        public override string Name => "Voe";
        public override string MainUrl => "voe.sx";

        private readonly HttpClientService _http;
        private readonly AngleSharpHtmlParser _parser;
        private readonly ILogger<VoeExtractor> _logger;

        public VoeExtractor(HttpClientService http, AngleSharpHtmlParser parser, ILogger<VoeExtractor> logger)
        {
            _http = http;
            _parser = parser;
            _logger = logger;
        ...
        {
            _logger.LogInformation("[Voe] Extracting {Link}", link);
            ...
            throw new System.Exception("Voe: no source found");
        }
    }
}
